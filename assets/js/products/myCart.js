...
.then((data) => {
  ...
  let arrayPrice = [];
    ...
    for (const key in data.cart) {
      ...
      const child3 = addElement("select",[],{name : item.sizeId,  value: item.size});

      for (const index in data.size[key]) {
        const valueSize = data.size[key][index].size;
        const valueSizeId = data.size[key][index].id;
        const child_3 = addElement("option",[],{ value : valueSizeId }, `${valueSize}`);
        valueSize === item.size ? child_3.selected = "true" : ""; 
        child3.append(child_3);
      }
      const child4 = addElement("input",[ "w-20"],{type : "number", value : item.quantity}, `${item.quantity}`);
      const child5 = addElement("div",[],{}, `${item.price.toFixed(2)} €`);
      const child6 = addElement("div",[],{}, `${totalPrice.toFixed(2)} €`);
      const child7 = addElement("button",["btnDelete"],{type: "button", name : item.id, id : item.id,"data-modal-target":`popup-modal${-item.id}`, "data-modal-toggle": `popup-modal${-item.id}`}, "supprimer");
      const child_7 = addElement("div", ["fixed", "top-0", "left-0", "right-0", "z-50", "hidden", "p-4", 
      "overflow-x-hidden", "overflow-y-auto", "md:inset-0", "h-[calc(100%-1rem)]", "max-h-full"], {id : `popup-modal${-item.id}`, tabindex : "-1"});
      ...
      dad.append(child0, child1, child2, child3, child4, child5, child6, child7,child_7);
    ...
    const child8 = addElement("div",["mt-5"],{}, `prix hors livraison : ${finalPrice()} €`);
    const child9 = addElement("button", [], {type : 'submit'}, "Valider mon panier");
